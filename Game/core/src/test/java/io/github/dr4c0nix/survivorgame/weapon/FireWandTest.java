...
/**
 * Tests unitaires pour {@link FireWand}.
 *
 * Même approche que {@link SwordTest} : on vérifie la position de spawn du projectile
 * et la direction par défaut lorsque la direction du joueur est nulle.
 *
 * Utilise un {@link EntityFactory} mocké pour capturer les appels de création de projectile.
 */
public class FireWandTest {
    ...
    private FireWand fireWand;

    /**
     * Initialise les mocks et crée une instance de FireWand avant chaque test.
     */
    @Before
    public void setUp() {
        MockitoAnnotations.openMocks(this);
        fireWand = new FireWand(mockEntityFactory);
    ...
    /**
     * Vérifie la position de spawn du projectile lorsque le joueur regarde à droite.
     */
    @Test
    public void testUpdate_ProjectileSpawnPosition_FacingRight() {
        when(mockPlayer.getFacingDirection()).thenReturn(new Vector2(1, 0));

        fireWand.update(2f, mockPlayer); // delta > shotDelay (1.4) pour forcer le tir
